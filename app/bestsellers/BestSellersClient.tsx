...
import ComicsCenterCarousel, { type ComicsCenterSlide } from "@/src/components/category/ComicsCenterCarousel";
...
export default function BestSellersClient({
  ...
  carouselSlides: ComicsCenterSlide[];
}) {
  ...
  return (
    <div className="min-h-screen bg-black">
      <ComicsCenterCarousel slides={carouselSlides} />
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6 lg:gap-8 mb-8 mx-auto">
              {displayedComics.map((comic) => (
                <ProductCard key={String(comic.id)} comic={comic} showSold={true} cardWidth="w-full" />
              ))}
            </div>
